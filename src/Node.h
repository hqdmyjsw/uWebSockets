...
public:
    ...
    static void accept_cb(uv_poll_t *p, int status, int events) {
        ...
        uv_os_sock_t clientFd = accept(serverFd, (sockaddr *) &listenData->listenAddr, &listenAddrLength);
    ...
    // needs to add support for listening with ipv6
    template <void A(Socket s)>
    bool listen(int port, uS::TLS::Context sslContext, uS::NodeData *nodeData, void *user) {
        ...
        listenData->listenAddr.sin_family = AF_INET;
        listenData->listenAddr.sin_addr.s_addr = INADDR_ANY;
        listenData->listenAddr.sin_port = htons(port);
        ...
        if (bind(listenFd, (sockaddr *) &listenData->listenAddr, sizeof(sockaddr_in)) || ::listen(listenFd, 10)) {
            delete listenPoll;
            delete listenData;
            ::close(listenFd);
            return true;
